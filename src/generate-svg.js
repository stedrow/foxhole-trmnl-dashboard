...
`;
    ...
    for (const [regionName, data] of this.mapData) {
      if (data.regionGeometry) {
        ...
        console.log(`Skipping region ${regionName} - no geometry`);
      }
    }
    console.log(`Generated ${regionCount} regions`);
                    ...
                    if (isInside) {
                      ...
                      for (const possibleId of possibleIds) {
                        if (possibleId in this.conquerStatus.features) {
                          ...
                          console.log(
                            `Found conquerStatus for "${townName}" using ID "${possibleId}": team=${conquerFeature.team}, lastChange=${conquerFeature.lastChange}`,
                          );
                          break;
                        }
                      }
                      ...
                      if (!conquerFeature) {
                        ...
                        if (voronoiMatch) {
                          ...
                          console.log(
                            `Found conquerStatus for "${townName}" via voronoi property: team=${conquerFeature.team}, lastChange=${conquerFeature.lastChange}`,
                          );
                        }
                      }
                      ...
                      if (!conquerFeature) {
                        ...
                        if (notesMatch) {
                          ...
                          console.log(
                            `Found conquerStatus for "${townName}" via notes property: team=${conquerFeature.team}, lastChange=${conquerFeature.lastChange}`,
                          );
                        }
                      }
                      ...
                      if (conquerFeature) {
                        ...
                        console.log(
                          `No conquerStatus found for town "${townName}" in Voronoi region "${voronoiRegion.properties.notes}"`,
                        );
                      }
                    }
                  }
                  ...
                  for (const town of conquerableTowns) {
                    ...
                    if (isInside) {
                      ...
                      if (trackedTown) {
                        ...
                        console.log(
                          `Found tracked data for "${this.warApi.iconTypes[town.iconType].notes}": team=${trackedTown.team}, lastChange=${new Date(trackedTown.lastChange).toISOString()}`,
                        );
                      } else {
                        console.log(
                          `No tracked data found for "${this.warApi.iconTypes[town.iconType].notes}"`,
                        );
                      }
                      break;
                    }
  ...
  getVoronoiControlStatus(voronoiRegion, dynamicData, regionName) {
    ...
    for (const town of conquerableTowns) {
      ...
      console.log(
        `Town ${this.warApi.iconTypes[town.iconType].notes} inside "${voronoiTownName}": ${isInside}`,
      );
    ...
    if (!associatedTown) {
      ...
      console.log(
        `No town found inside Voronoi region "${voronoiTownName}" in ${regionName} - returning neutral`,
      );
      return "neutral";
    }
    ...
    console.log(
      `Voronoi region "${voronoiTownName}" mapped to town "${this.warApi.iconTypes[associatedTown.iconType].notes}" (${associatedTown.teamId}) in ${regionName}`,
    );
    ...
    if (this.conquerStatus && this.conquerStatus.features) {
      ...
      if (conquerFeature) {
        console.log(
          `Found tracked data for "${this.warApi.iconTypes[associatedTown.iconType].notes}": team=${conquerFeature.team}, lastChange=${new Date(conquerFeature.lastChange).toISOString()}`,
        );
      } else {
        console.log(
          `No tracked data found for "${this.warApi.iconTypes[associatedTown.iconType].notes}" with ID ${townId}`,
        );
      }
    }
  ...
  getColorWithAlpha(teamId, conquerFeature = null) {
    ...
    if (conquerFeature && conquerFeature.lastChange) {
      ...
      console.log(
        `Town captured ${(timeSinceCapture / (1000 * 60 * 60)).toFixed(1)} hours ago - Alpha: ${alpha}`,
      );
    }
  ...
  convertTownToWorldCoordinates(town, regionGeometry) {
    ...
    console.log(
      `Town ${this.warApi.iconTypes[town.iconType].notes} at (${town.x}, ${town.y}) -> world (${worldX.toFixed(0)}, ${worldY.toFixed(0)})`,
    );
